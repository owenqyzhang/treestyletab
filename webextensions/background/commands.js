...
async function performTreeItemsDragDropWithMessage(message) {
  ...
  globalThis.__treestyletabBreadcrumb?.('dragdrop', `dragged ${draggedTabIds.join(',')} droppedOn ${message.droppedOn?.id ?? '-'} attachTo ${message.attachToId ?? '-'} before ${message.insertBefore?.id ?? '-'} after ${message.insertAfter?.id ?? '-'} action ${message.action} pin ${message.shouldPin ? 1 : 0} unpin ${message.shouldUnpin ? 1 : 0}`); // eslint-disable-line no-underscore-dangle
